...
# RRT parameters
RRT_MAX_ITERS        = 1500
RRT_STEP_SIZE        = 0.30   # m
RRT_GOAL_THRESHOLD   = 0.30   # m
RRT_GOAL_SAMPLE_RATE = 0.25   # fraction of samples that are exactly the goal
...
def astar(array, start, goal):
    ...
    Kept as a fallback, but main planner is now RRT.
    """
...
# RRT helpers (continuous in world coordinates)
def bresenham(x0, y0, x1, y1):
    """Integer Bresenham line between two grid cells."""
    dx = abs(x1 - x0)
...
def rrt_plan(start_xy, goal_xy, occ_grid, resolution, originX, originY):
    """
    Basic RRT in continuous world coordinates.
    occ_grid: 2D array, 1 = obstacle, 0 = free/unknown (we allow going through unknown).
    """
    h, w = occ_grid.shape

    def world_to_grid(wx, wy):
        gx = int((wx - originX) / resolution)
        gy = int((wy - originY) / resolution)
        if gx < 0 or gy < 0 or gx >= w or gy >= h:
            return None
        return gx, gy

    def cell_free(gx, gy):
        return 0 <= gx < w and 0 <= gy < h and occ_grid[gy, gx] == 0

    def segment_free(p1, p2):
        """Check collision along a line segment between two world points."""
        s = world_to_grid(p1[0], p1[1])
        e = world_to_grid(p2[0], p2[1])
        if s is None or e is None:
            return False
        x0, y0 = s
        x1, y1 = e
        for gx, gy in bresenham(x0, y0, x1, y1):
            if not cell_free(gx, gy):
                return False
        return True
    ...
    def sample_point():
        if np.random.rand() < RRT_GOAL_SAMPLE_RATE:
            return goal_xy
        rx = originX + np.random.rand() * w * resolution
        ry = originY + np.random.rand() * h * resolution
        return (rx, ry)
    ...
    def steer(from_p, to_p):
        ...
        return (from_p[0] + dx * ratio, from_p[1] + dy * ratio)

    # Main RRT loop
    for _ in range(RRT_MAX_ITERS):
        rnd = sample_point()
        idx_near = nearest_index(rnd)
        p_near = nodes[idx_near]
        p_new = steer(p_near, rnd)

        if not segment_free(p_near, p_new):
            continue
        ...
        if math.hypot(p_new[0] - goal_xy[0], p_new[1] - goal_xy[1]) < RRT_GOAL_THRESHOLD:
            if segment_free(p_new, goal_xy):
                # reconstruct path
                path = [goal_xy]
                cur = len(nodes) - 1
                while cur != -1:
    ...
    # Fallback: best node with line-of-sight to goal
    best_idx = None
    best_d = float('inf')
    for i, n in enumerate(nodes):
        if not segment_free(n, goal_xy):
            continue
        d = math.hypot(n[0] - goal_xy[0], n[1] - goal_xy[1])
        if d < best_d:
            best_d = d
            best_idx = i
...
def findClosestGroup(matrix, groups, current, resolution,
    ...
    """
    Choose best frontier cluster, and plan path to a viewpoint using RRT.
    ...
    for i in range(len(groups)):
        ...
        # RRT planning in world coordinates 
        world_path = rrt_plan(start_world, goal_world,
                              matrix, resolution, originX, originY)

        # optional: fallback to A* if RRT fails
        if not world_path:
...
def exploration(node, data, width, height, resolution, col, row, originX, originY):
    ...
    - plan with RRT (world coords), fallback to A*
    ...
    # for frontier detection and RRT:
    #   >50 -> obstacle (1)
...
class navigationControl(Node):
    def __init__(self):
        ...
        print("[INFO] Task1 exploration node (frontier + deep unknown goals + RRT + backup)")
